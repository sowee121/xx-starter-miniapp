...
| 点读音频       | 分包 MP3 + `utils/audio.js`；文件名一律 ASCII slug；真机需 `setInnerAudioOption`；播放失败落「语音准备中～」软提示 |
| 云端存储（图片） | 环境已配置；默认 `USE_CLOUD = false`（免费套餐 ACL）；手动/自动化见 [`../../cloud-assets/README.md`](../../cloud-assets/README.md) |
| 云函数         | **已部署**（含 `getProgress`）；启动同步积分/进度；学习完成走 `completeProgress`；失败本地兜底、云通畅冲刷 |
...
| P4 配套 | 日历、每日任务、贴纸商城（无勋章）           | §五 6/7/8   | 完成（任务数量已改为每日随机；反馈闭环：弹层 L1/L2/L3 + 行内 soft-note + 成败音效，禁 Toast） |
| P5 云端 | 云函数：积分/打卡/进度/贴纸；去本地唯一源      | §六         | **进行中**：积分/打卡/贴纸/学习进度（`completeProgress`+`getProgress`）已接通；加星与进度均本地失败兜底、云通畅冲刷；贴纸兑换仅云端 |
