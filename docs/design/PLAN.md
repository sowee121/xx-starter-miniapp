# 嘻嘻启蒙乐园 · 设计与开发规划（终审定稿）

> 唯一需求源：
>
> - `[小程序需求优化版.txt](../../小程序需求优化版.txt)`
> - `[小程序开发核对清单（最终验收版）.txt](../../小程序开发核对清单（最终验收版）.txt)`
> - 内容与视觉定稿：[CONTENT.md](./CONTENT.md) + `docs/design/h5/` + `docs/design/atoms/`
>
> 旧设计稿/素材已清空。本文件为执行基准；**题库与视觉已于 2026-08-16 定稿**，2026-08-17 起业务页与每日任务随机数量、云函数部署文档已对齐代码；业务样式与 H5 双向同步，再闭环云端（P5）。

---

## 0. 当前状态


| 项          | 状态                |
| ---------- | ----------------- |
| 需求文档       | 已定稿（根目录两份 + 本规划） |
| 内容题库       | **已定稿**：见 [CONTENT.md](./CONTENT.md) 与 `docs/design/content/*.json` |
| 设计稿 / 原子素材 | **H5 审查稿 + atoms 已确认可作视觉基准**（`docs/design/h5/`、`docs/design/atoms/`） |
| 业务页代码      | **八大板块已接入**；样式与 H5 双向同步（见仓库 `.cursor/rules/h5-miniapp-style-sync.mdc`） |
| 每日任务       | **已实现**：自由学习按数量累计；数量每天随机（古诗 1～2，字/题/词/拼音 1～5，日历固定 1）；实现 `miniprogram/utils/daily-tasks.js` |
| 点读音频       | 分包 MP3 + `utils/audio.js`；真机需 `setInnerAudioOption` + 中文路径 `encodeURI` |
| 云端存储（图片） | 环境已配置；默认 `USE_CLOUD = false`（免费套餐 ACL）；上传源见 `cloud-assets/` |
| 云函数         | 源码在 `cloudfunctions/`；**须工具内绑环境并上传**（见 [`../../cloudfunctions/README.md`](../../cloudfunctions/README.md)）；客户端失败时仍本地兜底 |


---

## 1. 产品锁定

- **名称（产品内部 / 页内展示）**：嘻嘻启蒙乐园
- **名称（微信后台创建 / 搜索展示）**：嘻宝星屋（过审用对外名；申请文案见 `docs/apply/miniprogram-intro.md`）
- **人群**：2 岁半零基础低幼
- **形态**：微信原生小程序，移动端优先，`750rpx` 设计/实现基准
- **气质**：晨间草地柔光 + 圆角积木控件 + 全站黏土软萌素材/动物
- **原则**：护眼低饱和、极简操作、低难度、全正向激励、零负面反馈、无广告/付费/弹窗、云端永久存储

---

## 2. 全局 UI / 壳层硬约束

### 2.1 视觉

- 背景：全站晨间浅绿柔光草地（淡嫩绿 + 柔光白）；仅静态浅云/光斑/草纹理，无闪烁动效
- 控件：超大圆角积木；微立体、柔边浅阴影；马卡龙低饱和色块；无尖锐角、无复杂描边纹理
- 视觉基准稿：**已定稿** — `docs/design/h5/`（审查页）+ [CONTENT.md](./CONTENT.md) + `docs/design/atoms/`
- 素材：100% 高清黏土捏塑（果蔬、物品、景物、动物、纯字母）；禁止手绘/扁平/写实照片/二次元；识字详情允许系统 emoji 表意
- 色彩：主色晨间嫩草绿；辅色奶油白、浅奶黄、蜜桃浅粉、天空浅蓝；禁用纯黑、正红、亮黄等高对比刺眼色
- 字体：圆润加粗、字号偏大；无细字、花体、密集长文



### 2.2 布局与热区

- **无左侧栏、无底部 Tab**；八大功能在首页平铺大积木卡
- 同级卡片、选项、入口等均匀排列优先使用 **`display: flex` + `gap`**；不要用子项 margin 或 `space-between` 模拟间距
- 点击热区 ≥ **120rpx × 120rpx**
- 圆角：按钮 / pill / chip **24–40rpx**；卡片 **36–48rpx** 超大圆角（相对需求原文「卡片 20rpx」的产品侧优化；取范围内值即可，同级控件全站一致，不强制全站同一数字）
- 模块间距约 **30rpx**，留白宽松
- 适配刘海/底部安全区；有效操作避开系统 UI



### 2.3 自定义导航（高 88rpx）

- 关闭原生默认头；全站自定义导航
- 左：回首页；中：标题；右：星星积分
- **工程落实**：右侧积分必须落在微信胶囊**左侧安全区**（需求文案「右侧展示」，实现按胶囊避让，禁止与胶囊重叠）
- **首页独有**（内页不显示）：黏土头像 + 欢迎语「宝贝，你好呀～一起快乐学习吧！」，圆角积木卡



### 2.4 交互与语音

- 仅支持**选择题、拖拽**；禁止手写/键盘/拼写
- 答错：无扣分、无红字、无刺耳音、无负面话术；仅温柔正向鼓励
- 点读：慢速标准童声；文字/图/词/诗句支持单点独立发声



### 2.5 积分与奖励

- 学习、答题、打卡获星；只增不减、不清零、不扣除
- 顶部积分实时刷新
- **仅**兑换黏土虚拟贴纸；**无勋章、无成就体系**
- 贴纸资产云端保存



### 2.6 数据

- 积分、打卡、学习进度、贴纸 → **微信云函数云端存储**
- 换机/清缓存不丢数；禁止纯本地缓存作为唯一数据源

---



## 3. 八大功能板块


| #   | 板块   | 范围要点                                                     |
| --- | ---- | -------------------------------------------------------- |
| 1   | 古诗   | 6 首指定（咏鹅、静夜思、悯农、春晓、登鹳雀楼、望庐山瀑布）；大字原文；逐句点读 + 全文朗读；4:3 黏土场景封面；**无白话字段/展示** |
| 2   | 识字   | 古诗库 21（笔画升序）+ 生活库 30（一～十数值序在前，其后关联分组；无「鱼」）；详情系统 emoji；点读 + 1～2 组词 |
| 3   | 算术   | 数一数 10 题：水果池随机拼接，Grid 定列（10=上5下5）；算一算 10 题：固定狗拿算盘图 |
| 4   | 英语   | 20 词（水果/动物/颜色/身体/交通各 4）+ 超短句；黏土词图；词/句独立点读 |
| 5   | 拼音   | **仅单韵母** a o e i u ü；纯黏土字母图；点读；无声母、无拼读 |
| 6   | 日历   | 日期、星期、昼夜；晨间草地 + 黏土日/月/云/动物                               |
| 7   | 每日任务 | 六模块各 1 条；数量每天随机（古诗 1～2，字/题/词/拼音 1～5，日历固定 1）；自由学习按数量累计，点读或答对自动完成；任务页只展示进度、不可点勾；星星按数量（古诗 +1） |
| 8   | 积分商城 | 星星换黏土贴纸；无勋章                                              |


**明确不做**：运动、思维游戏、录音跟读评分、家长区独立模块、勋章墙、弹窗广告付费会员。

### 动物素材（验收）

- 核心常驻：猫、狗、小鸭、兔子、小熊、绿色可爱小恐龙
- 功能道具映射：古诗—书、识字—字卡、算术—算盘、英语—字母积木/书、拼音—纯字母（无动物道具）、日历—日历牌、每日任务—星星清单、积分商城—贴纸
- 扩展：鸡、羊、牛、猪、小鸟、狐狸、熊猫、大象、长颈鹿、狮子、老虎、猴子、企鹅、海豚等（温和无凶狠）

**内容定稿全文** → [CONTENT.md](./CONTENT.md)

---



## 4. 工作流（当前）

```mermaid
flowchart LR
  contentDone[CONTENT + H5 + atoms 定稿] --> shell[壳层与首页按稿重建]
  shell --> modules[八大板块分批接入]
  modules --> cloud[云函数闭环]
  cloud --> accept[验收清单终验]
```



**硬门禁**：执行以 [CONTENT.md](./CONTENT.md)、`docs/design/h5/`、`docs/design/atoms/` 为准。未确认的**新**视觉/题库改动：先改 JSON + CONTENT，再重生成 H5 确认，禁止开发期现场另起风格。

---



## 5. 设计阶段（已完成）


| 阶段     | 产出                            | 目录 / 状态                                            | 对照验收     |
| ------ | ----------------------------- | ----------------------------------------------- | -------- |
| D1 母版  | 首页 / 列表 / 详情视觉语言               | 已收敛进 H5 审查帧                    | 清单 §一–§三 |
| D2 全套  | 八大板块静态审查页（约 45 帧）            | **已定稿** `docs/design/h5/`                      | 清单 §五、§八 |
| D3 原子包 | 草地、动物、果蔬、封面、拼音字母等透明底切图    | **已归档** `docs/design/atoms/` → 开发时同步 `miniprogram/static/` | 清单 §七    |


禁止开发期现场抠图另起风格；新增原子须走 chroma 去背 + normalize 流程。

---



## 6. 开发阶段（设计确认后）


| 阶段    | 内容                          | 对照验收       | 进度 |
| ----- | --------------------------- | ---------- | --- |
| P0 壳层 | 自定义导航、安全区、欢迎卡、八入口、星星位       | §一、§三      | 完成 |
| P1 点读 | 统一播放工具；古诗/识字/英语/拼音接入        | §四         | 完成 |
| P2 内容 | 6 首古诗 → 识字 21+30 → 拼音单韵母 → 英语 20 词句（见 CONTENT.md） | §五 1/2/4/5 | 完成 |
| P3 互动 | 数物拼接 + 1–5 加减；仅选择；正向反馈   | §五 3、§四    | 完成 |
| P4 配套 | 日历、每日任务、贴纸商城（无勋章）           | §五 6/7/8   | 完成（任务数量已改为每日随机） |
| P5 云端 | 云函数：积分/打卡/进度/贴纸；去本地唯一源      | §六         | **进行中**：源码就绪，待上传部署并去掉「仅本地」兜底为唯一路径 |
| P6 收尾 | 懒加载、机型与安全区、少儿纯净项            | §一         | 未开始 |


---



## 7. 验收对照（最终必过）

直接使用根目录《小程序开发核对清单（最终验收版）.txt》八章作为打勾标准：

1. 基础整体规范
2. 全局 UI 视觉
3. 顶部导航 & 首页欢迎模块
4. 交互 & 答题机制
5. 八大功能板块
6. 云端数据存储
7. 开发前置设计交付
8. 积分激励规则

---



## 8. 与历史做法的差异（防回潮）


| 旧习惯                      | 终审要求                          |
| ------------------------ | ----------------------------- |
| 视觉基准未锁定               | **H5 + CONTENT.md 已定稿**（2026-08-16） |
| 热区 96rpx                 | ≥ 120rpx                      |
| 星星放在 Header 下方信息行        | 导航栏右侧展示（避让胶囊）                 |
| 奖励含勋章墙                   | **仅贴纸**                       |
| 含家长区 / 运动 / 思维           | **不做**                        |
| 古诗数量开放                   | **6 首指定**；无白话字段/展示 |
| 识字随意排序 / 含鱼             | 古诗库笔画序；生活库数字在前+关联序；无「鱼」 |
| 拼音动物拿卡                   | **纯黏土单韵母字母** |
| 数一数固定一张合成图           | 水果原子拼接；10 个上5下5 |
| 算一算随机动物图               | **固定狗拿算盘** |
| 本地 storage 为主            | **目标：云函数云端**；过渡期 `stars.js` 失败仍本地兜底 |
| 每日任务固定条数文案             | **每天随机数量**（古诗 1～2，其余学习 1～5）；见 CONTENT §2.6 |

---



## 9. 设计确认门禁

1. **内容与 H5 审查稿已于 2026-08-16 用户确认定稿**，执行以 [CONTENT.md](./CONTENT.md) + `docs/design/h5/` + `docs/design/atoms/` 为准。
2. 未确认的**新**视觉改动：仍须先出设计图，确认后再改 `PLAN.md` / `CONTENT.md` 与业务样式。
3. 题库变更：先改 `docs/design/content/*.json` 与 CONTENT.md，再 `python3 scripts/generate_h5_inner_pages.py` 重生成审查页。
4. 进入开发后：按定稿重建业务页样式与素材引用；禁止开发期现场抠图另起风格。

