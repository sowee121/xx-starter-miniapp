...
| 要素 | 要求 | Token / 类 |
| --- | --- | --- |
| 轮廓 | 卡片用**普通圆弧** `border-radius`（**不用** `corner-shape: squircle`，小程序 WebView 无法稳定还原）；**石子径单字正圆**（`999`）；切题用矮条卡 + 黏土箭头 | `--radius-card` / `--radius-tile` / `--radius-bar` / `--radius-thumb` / `--radius-pill`、`.trail-nav__item` / `.is-step` |
...
- **切题箭头**：详情底栏左右矮条卡用 `--tone-cream` + `--shadow-clay`（对齐组词/诗行/例句），内放 `arrow.png`；上一题 `scaleX(-1)`。禁止绿胶囊文案钮，禁止「圆盘+箭头」整钮 PNG。
...
| 数一数固定一张合成图           | 固定苹果原子拼接数量；10 个上 5 下 5 |
| 算一算随机动物图               | **固定狗拿算盘** |
